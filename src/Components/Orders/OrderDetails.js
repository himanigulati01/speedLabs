...
function OrderDetailsPage(props) {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const orderList = orderArray(orderJson.orders, "order_id");
      console.log(orderList);
      const orderData = orderList.find((orderItem) => orderItem[1].order_id === id);
      if(orderData)
        setData(orderData);
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div class="container">
      ...
      {errorOrderDetail && <MessageBox variant="danger">{errorOrderDetail}</MessageBox>}
      {data && data[1] &&
      <div>
      <div class="row">
        <div class="col-md-6">
          <ul className="order_list">
          {data[1].order_status === "processed" ? <li><strong>Order Status</strong> <i className="success-status">Processed</i></li>:
          <li><i className="pending-status">Pending</i></li>
          }
          <li><strong>Order Id</strong> {data[1].order_id}</li>
          <li><strong>Issued On</strong> {data[1] && (moment(data[1].issued_on).local().format('DD-MM-YYYY   HH:mm:ss'))}</li>
          </ul>
        </div>

        <div class="col-md-6">
              <ul className="order_list">
                  <li><strong>Mode of Payment</strong> {data && data[1].method_of_payment}</li>
                  <li><strong>Payment Id</strong> {data && data[1].payment_id}</li>
                  <li><strong>Invoice Number</strong> {data && data[1].invoice_number}</li>
              </ul>
          </div>
      </div>
                        ...
                        {data[1].product.map((item)=>(
                            <tr>
                              ...
                              <td data-title="Total">
                                ...
                                <td data-title="Actions"><a><i className="fa fa-fw fa-eye"></i></a></td>
                            </tr>
                  ...
                  <table class="table table-cart-total">
                    <tbody>
                      <tr>
                        ...
                        <td>
                          <div class="price">₹{(data[1].product.reduce((a, curr) => a + (curr.price_before_coupon ? (curr.price_before_coupon) : 0), 0)).toFixed(2)}</div>
                        </td>
                      </tr>
                      <tr>
                        <td class="font-lato fw-bold">Discount</td>
                        <td>
                          <div lass="price">₹{((data[1].product.reduce((a, curr) => a + (curr.price_before_coupon ? (curr.price_before_coupon) : 0), 0)) - data[1].amount).toFixed(2) }</div>
                        </td>
                      </tr>
                      ...
                      <tr className="yellow-border">
                        <td>Total</td>
                        <td >
                          <strong class="price">₹{(data[1].amount).toFixed(2)}</strong>
                        </td>
                      </tr>
                    </tbody>
                  </table>
